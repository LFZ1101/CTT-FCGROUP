...
Embeddings locais `hashing-v1` (256-d) em `DocumentChunk.embedding`. Retrieval híbrido (Jaccard + cosseno). Síntese OpenAI opcional via `OPENAI_API_KEY`. Recusa sem evidência.
